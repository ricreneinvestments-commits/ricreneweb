...
interface Project  { id: number; name: string; service: string; status: string; amount: number; start_date: string | null; end_date: string | null; description: string; }
...
interface Payment  { id: number; plan_name: string; amount: number; billing_period: string; status: string; created_at: string; }

// ── Helpers ───────────────────────────────────────────────────────────────────

const statusColors: Record<string, string> = {
  inquiry: "bg-gray-100 text-gray-600", proposal: "bg-blue-50 text-blue-600",
  active: "bg-green-50 text-green-700", review: "bg-yellow-50 text-yellow-700",
  completed: "bg-emerald-50 text-emerald-700", on_hold: "bg-orange-50 text-orange-600",
  ...
  cancelled: "bg-gray-100 text-gray-400", pending: "bg-yellow-50 text-yellow-700",
  confirmed: "bg-blue-50 text-blue-600",
};
const statusLabels: Record<string, string> = {
  inquiry: "Inquiry", proposal: "Proposal Sent", active: "Active", review: "In Review",
  completed: "Completed", on_hold: "On Hold", draft: "Draft", sent: "Sent",
  paid: "Paid", overdue: "Overdue", cancelled: "Cancelled", pending: "Pending", confirmed: "Confirmed",
};
...
function EmptyState({ message, cta, href }: { message: string; cta?: string; href?: string }) {
  return (
    <div className="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
      ...
      {cta && href && <Link href={href} className="text-sm text-red-600 font-medium hover:underline">{cta} →</Link>}
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const handleProjectRequest = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/api/projects/request/`, {
        ...
        body: JSON.stringify({ name: projName, service: projService, description: projDesc, budget: projBudget }),
      });
      if (res.ok) {
    ...
    } catch { setProjMsg("Failed to submit. Please try again."); }
  ...
  const handleMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else { setMsgStatus("Failed to send. Please try again."); }
    } catch { setMsgStatus("Failed to send. Please try again."); }
  ...
  const handlePrintInvoice = (inv: Invoice) => {
    ...
    win.document.write(`
      ...
      <style>body{font-family:sans-serif;padding:40px;max-width:600px;margin:0 auto}h1{color:#dc2626}table{width:100%;border-collapse:collapse;margin-top:20px}td,th{padding:10px;border:1px solid #e5e7eb;text-align:left}.total{font-weight:bold;font-size:1.1em}</style>
      </head><body>
      ...
      <table>
        ...
        <tr><td>${inv.project_name}</td><td>${fmt(inv.amount)}</td></tr>
        <tr><td colspan="1" class="total">Total</td><td class="total">${fmt(inv.amount)}</td></tr>
      </table>
        ...
        <div className="max-w-5xl mx-auto px-6 pt-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <form onSubmit={handleMessage} className="space-y-4">
              ...
              />
              {msgStatus && <p className={`text-sm ${msgStatus.includes("sent") ? "text-green-600" : "text-red-600"}`}>{msgStatus}</p>}
              <div className="flex gap-3">
                <button type="submit" disabled={msgSubmitting} className="px-5 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 disabled:opacity-60 transition-colors">
                  {msgSubmitting ? "Sending..." : "Send Message"}
                </button>
                <button type="button" onClick={() => setActivePanel(null)} className="px-5 py-2 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-10">
              ...
              <form onSubmit={handleProjectRequest} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={projName} onChange={e => setProjName(e.target.value)} required placeholder="e.g. Company Website" className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5">Service needed *</label>
                    <select value={projService} onChange={e => setProjService(e.target.value)} required className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 bg-white">
                ...
                <div>
                  ...
                  <textarea value={projDesc} onChange={e => setProjDesc(e.target.value)} required rows={3} placeholder="Describe what you need..." className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">Budget (optional)</label>
                  <input value={projBudget} onChange={e => setProjBudget(e.target.value)} placeholder="e.g. TZS 500,000 or Not sure" className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                </div>
                {projMsg && <p className={`text-sm ${projMsg.includes("submitted") ? "text-green-600" : "text-red-600"}`}>{projMsg}</p>}
                <div className="flex gap-3">
                  <button type="submit" disabled={projSubmitting} className="px-5 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 disabled:opacity-60 transition-colors">
                ...
                <div key={p.id} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">{p.service}</p>
                      {p.description && <p className="text-sm text-gray-400 mt-1 line-clamp-2">{p.description}</p>}
                    </div>
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <Badge s={p.status} />
                      {p.amount > 0 && <p className="text-sm font-medium text-gray-700">{fmt(p.amount)}</p>}
                    </div>
                  </div>
                </div>
              ))}
                ...
                <tbody className="divide-y divide-gray-100">
                  {invoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-gray-500 hidden sm:table-cell">{inv.project_name}</td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <button onClick={() => handlePrintInvoice(inv)} className="text-red-600 hover:text-red-700 text-xs font-medium hover:underline">
                          Download
                        </button>
                      </td>
                    </tr>
                ...
                <div key={pay.id} className="bg-white rounded-xl border border-gray-200 p-5 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <p className="text-sm font-medium text-gray-700 hidden sm:block">
                      {fmt(pay.amount)}<span className="text-gray-400 font-normal ml-1">/ {pay.billing_period}</span>
                    </p>
                    <Badge s={pay.status} />
                  </div>
                </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            {!editProfile && (
              <button onClick={() => setEditProfile(true)} className="text-sm text-red-600 font-medium hover:text-red-700 transition-colors">Edit</button>
            )}
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            {editProfile ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={firstName} onChange={e => setFirstName(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5">Last name</label>
                    <input value={lastName} onChange={e => setLastName(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">Phone</label>
                  <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="+255 700 000 000" className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">Company name</label>
                  <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Optional" className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                </div>
                {profileMsg && <p className={`text-sm ${profileMsg === "Saved!" ? "text-green-600" : "text-red-600"}`}>{profileMsg}</p>}
                <div className="flex gap-3">
                  <button onClick={handleSaveProfile} disabled={profileSaving} className="px-5 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 disabled:opacity-60 transition-colors">
                    {profileSaving ? "Saving..." : "Save changes"}
                  </button>
                  <button onClick={() => { setEditProfile(false); setProfileMsg(""); }} className="px-5 py-2 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
                </div>
              </div>
            ) : (
